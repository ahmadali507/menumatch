...
export default function MenuCard({ menu }: MenuCardProps) {
  const languageName =
    SUPPORTED_LANGUAGES.find((l) => l.code === menu.language)?.name ||
    menu.language;
    ...
    switch (menu.availabilityType) {
      case "indefinite":
        return {
          ...
          statusText:
            menu.status === "active" ? "Always Available" : "Inactive",
  ...
  return (
    <StyledCard>
            ...
            <Link
              href={
                user?.role == "super_admin"
                  ? `/restaurants/${restaurantId}/menu/${menu.id}`
                  : `${routes.menu}/${menu.id}`
              }
              passHref
            >
      ...
      <Stack spacing={2} sx={{ height: "100%", minHeight: "160px" }}>
        {" "}
        {/* Added minHeight */}
        <Stack spacing={1} sx={{ flex: 1 }}>
          ...
          <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
            ...
            <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
              <AccessTimeIcon color={menuStatus.color} sx={{ fontSize: 20 }} />
              <Typography
                variant="caption"
                sx={{
              ...
              {menuStatus.status !== "invalid" && (
                ...
                >
                  {menuStatus.status === "active"
                    ? `Expires ${formatDistanceToNow(menu.endDate as Date, {
                        addSuffix: true,
                      })}`
                    : menuStatus.status === "upcoming"
                    ? `Starts ${formatDistanceToNow(menu.startDate as Date, {
                        addSuffix: true,
                      })}`
                    : `Expired ${formatDistanceToNow(menu.endDate as Date, {
                        addSuffix: true,
                      })}`}
                </Typography>
              )}
